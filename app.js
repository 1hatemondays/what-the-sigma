...
function wheelGraphic(spinning = false, item = 'shield') { const items = ['shield', 'hint', 'bonus', 'fog', 'freeze', 'trap']; const offset = -60 * Math.max(0, items.indexOf(item)); return `<div class="wheel-wrap" aria-hidden="true"><span class="wheel-pointer"></span><div class="wheel ${spinning ? 'spinning' : ''}" style="--final-angle:${1440 + offset}deg"><span class="wheel-label one">KHIÊN</span><span class="wheel-label two">GỢI Ý</span><span class="wheel-label three">CỘNG ĐIỂM</span><span class="wheel-label four">MÀN SƯƠNG</span><span class="wheel-label five">ĐÓNG BĂNG</span><span class="wheel-label six">BẪY ĐIỂM</span></div></div>`; }
...
function questionPage() {
  ...
  const host = state.role === 'host';
  const canSkip = host || session.demo;
  const fog = !host && state.me.fogUntil > nowServer();
  const frozen = !host && state.me.freezeUntil > nowServer();
  ...
  const image = q.images?.length ? `<figure class="image-question" style="${imageRevealStyle()}"><div class="image-frame ${q.images.length > 1 ? 'image-pair' : ''}" style="aspect-ratio:auto;align-items:start">${q.images.map((src, index) => `<img src="${esc(src)}" alt="${esc(q.imageAlt)}${q.images.length > 1 ? `, ảnh ${index + 1}` : ''}" style="width:100%;height:auto;object-fit:contain;transform:none">`).join('')}<span class="image-veil" aria-hidden="true"></span></div><figcaption>Hình ảnh sẽ hiện rõ dần theo thời gian</figcaption></figure>` : '';
  const attackStatus = frozen ? '<div class="feedback wrong attack-status">❄ Nhóm đang bị Đóng băng trong 3 giây đầu.</div>' : state.me?.trapArmed ? '<div class="feedback wrong attack-status">⚠ Bẫy điểm đang hoạt động: trả lời sai lần đầu sẽ bị trừ 100 điểm.</div>' : '<div class="feedback wrong attack-status hide"></div>';
  const form = host ? '<p class="hint-line">Câu hỏi sẽ tự kết thúc khi hết giờ hoặc tất cả nhóm trả lời đúng.</p>' : state.me.solved ? `<div class="feedback correct">Nhóm bạn đã trả lời đúng và nhận ${state.me.roundPoints} điểm. Chờ các nhóm khác.</div>` : `${attackStatus}<form id="answer-form" class="answer-form"><input class="input" id="answer-input" name="answer" maxlength="100" autocomplete="off" autocapitalize="sentences" placeholder="Nhập đáp án của nhóm..." aria-label="Nhập đáp án" ${frozen ? 'disabled' : ''} required><button class="btn" type="submit" ${wrongWait || frozen ? 'disabled' : ''}>Trả lời →</button></form>${feedback}`;
  const skipControl = canSkip ? '<div class="host-actions"><button class="btn ghost" data-action="skip-question">Bỏ qua câu hỏi →</button></div>' : '';
  const main = `<div class="panel"><div class="timer-row"><div><div class="kicker">Câu ${state.questionIndex + 1} / ${state.totalQuestions}</div><div class="muted">${q.images?.length ? 'Quan sát hình ảnh và đoán đáp án' : 'Mỗi giây hé một ký tự'}</div></div><div class="timer ${timeLeft <= 5 ? 'urgent' : ''}">${String(timeLeft).padStart(2, '0')}<small> giây</small></div></div><div class="timer-track"><div class="timer-fill" style="width:${percent}%"></div></div><div class="score-meter"><div class="score-meter-head"><span>${scoreLabel}${scoreMeter.bonus ? ' · Đã cộng 200 điểm chức năng' : ''}</span><strong class="live-score">${scoreMeter.score} điểm</strong></div><div class="score-track"><div class="score-fill" style="width:${scoreMeter.percent}%"></div></div></div><h3 class="question-heading">${esc(q.prompt)}</h3>${image}<div class="slots ${fog ? 'fogged' : ''}" aria-label="Ô chữ đáp án">${slotsHtml(q.slots)}</div>${fog ? '<p class="hint-line">Màn sương đang che ô chữ. Bạn vẫn có thể nhập đáp án.</p>' : ''}${form}${skipControl}</div>`;
...
function revealPage() {
  ...
  const image = q.images?.length ? `<figure class="image-question revealed" style="${imageRevealStyle(1)}"><div class="image-frame ${q.images.length > 1 ? 'image-pair' : ''}" style="aspect-ratio:auto;align-items:start">${q.images.map((src, index) => `<img src="${esc(src)}" alt="${esc(q.imageAlt)}${q.images.length > 1 ? `, ảnh ${index + 1}` : ''}" style="width:100%;height:auto;object-fit:contain;transform:none">`).join('')}</div><figcaption>Hình ảnh gợi ý đã được mở hoàn toàn</figcaption></figure>` : '';
...
function updateQuestionDynamic() {
  ...
  const frozen = Boolean(state.me?.freezeUntil > nowServer());
  const answerInput = document.getElementById('answer-input');
  if (answerInput) answerInput.disabled = frozen;
  const submit = document.querySelector('#answer-form button[type=submit]');
  if (submit) submit.disabled = frozen || Boolean(state.me?.wrongUntil > nowServer());
  const attackStatus = document.querySelector('.attack-status');
  if (attackStatus) {
    attackStatus.classList.toggle('hide', !frozen && !state.me?.trapArmed);
    attackStatus.textContent = frozen ? '❄ Nhóm đang bị Đóng băng trong 3 giây đầu.' : state.me?.trapArmed ? '⚠ Bẫy điểm đang hoạt động: trả lời sai lần đầu sẽ bị trừ 100 điểm.' : '';
  }
}
...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    if (name === 'skip-question') { button.disabled = true; await action('skip', {}, session.demo ? session.demoHostToken : undefined); return; }
...
document.addEventListener('submit', async event => {
  ...
  try {
    if (form.id === 'host-gate-form') {
      ...
      if (!result.correct) notify(result.feedback?.text || 'Chưa đúng. Bạn có thể thử lại sau 2 giây.');
